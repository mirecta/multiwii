...
#elif defined(MIRC)
  volatile uint16_t rcValue[RC_CHANS] = {1502, 1502, 1502, 1502, 1502, 1502, 1502, 1502,1502,1502}; // interval [1000;2000]
#else
...
#elif defined(MIRC)
  static uint8_t rcChannel[RC_CHANS] = {ROLL,PITCH,YAW,THROTTLE,AUX1,AUX2,AUX3,AUX4,AUX5, AUX6};
...
uint16_t readRawRC(uint8_t chan) {
  uint16_t data;
  #if defined(SPEKTRUM) || defined(SBUS) || defined(MIRC)
    if (chan < RC_CHANS) {
...
void computeRC() {
  ...
  uint8_t chan,a;
    ...
    for (chan = 0; chan < RC_CHANS; chan++) {
      ...
      #endif
      #if defined(SPEKTRUM) || defined(SBUS) || defined(MIRC) // no averaging for Spektrum & SBUS signal
...
void initMiRC(void) {
  ...
  MIRC_MISO_PORTDDR &= ~_BV(MIRC_MISO_PIN);
  MIRC_MOSI_PORTDDR |= _BV(MIRC_MOSI_PIN);
  MIRC_SCK_PORTDDR |= _BV(MIRC_SCK_PIN);
  MIRC_SS_PORTDDR |= _BV(MIRC_SS_PIN);
  
  MIRC_SS_PORTOUT |= _BV(MIRC_SS_PIN);
  MIRC_SCK_PORTOUT &= ~_BV(MIRC_SCK_PIN);
  MIRC_MOSI_PORTOUT &= ~_BV(MIRC_MOSI_PIN);
}


byte sendReceiveMiRC(byte data = 0x00){
 ...
 for (int i = 7; i >=0 ; --i){
  ...
  NOP();
  //read input
  if ((MIRC_MISO_PORTIN & _BV(MIRC_MISO_PIN)) != 0)
        result |= _BV(i);
  //clock down
  MIRC_SCK_PORTOUT &= ~_BV(MIRC_SCK_PIN);
  NOP();
...
/*
  ...
  {ROLL,PITCH,YAW,THROTTLE,AUX1,AUX2,AUX3,AUX4,AUX5,AUX6};
  */
...
void readMiRC(void) {
 ...
 NOP();
 NOP();
 sendReceiveMiRC(0xc0);
 for( char i = 0; i < 14; ++i) {
  
  buffer[i] = sendReceiveMiRC(); 
   
 }
 
 rcValue[0] = buffer[7] | ((buffer[8] & 0x0f) << 7); //roll
 rcValue[1] = buffer[1] | ((buffer[2] & 0x0f) << 7); //pitch
 rcValue[2] = buffer[3] | ((buffer[4] & 0x0f) << 7); //yaw
 rcValue[3] = 3000 - (buffer[5] | ((buffer[6] & 0x0f) << 7)); // throttle
 rcValue[4] = buffer[9] | ((buffer[10] & 0x0f) << 7); // aux1
 
 if ((buffer[2]) & (1 << 6)) rcValue[5] = 2000; else rcValue[5] = 1000; // aux 2
 if ((buffer[2]) & (1 << 5)) rcValue[6] = 2000; else rcValue[6] = 1000; // aux 3
 if ((buffer[2]) & (1 << 4)) rcValue[7] = 2000; else rcValue[7] = 1000; // aux 4
 
 if ((buffer[4]) & (1 << 6)) rcValue[8] = 2000; else rcValue[8] = 1000; // aux 5
 if ((buffer[4]) & (1 << 5)) rcValue[9] = 2000; else rcValue[9] = 1000; // aux 6
